...
public class DocumentUploadController {
	...
	public ResponseEntity<LamsResponse> upload(@RequestPart("uploadRequest") String documentRequestString, @RequestPart("file") MultipartFile multipartFiles,HttpServletRequest httpServletRequest) {
		...
		if (CommonUtils.isObjectNullOrEmpty(userId)) {
			...
			return new ResponseEntity(new LamsResponse(HttpStatus.BAD_REQUEST.value(), "Invalid Request, User id is null  or empty"), HttpStatus.OK);
		}
		try {
			...
			if ((CommonUtils.isObjectNullOrEmpty(documentRequest.getDocumentId()))) {
                ...
                return new ResponseEntity(new LamsResponse(HttpStatus.BAD_REQUEST.value(), "Invalid Request"), HttpStatus.OK);
            }
			...
			return new ResponseEntity(new LamsResponse(HttpStatus.OK.value(), "Successfully Uploaded File",documentResponse), HttpStatus.OK);
		} catch (Exception e) {
			logger.info("Throw Expection while Upload File -------------------->");
			e.printStackTrace();
			return new ResponseEntity(new LamsResponse(HttpStatus.INTERNAL_SERVER_ERROR.value(), "Something went wrong while upload file"), HttpStatus.INTERNAL_SERVER_ERROR);
		}
	...
	public ResponseEntity<LamsResponse> getDocuments(@PathVariable("applicationId") Long applicationId,@RequestBody List<Long> documentList) {
		...
		if (CommonUtils.isListNullOrEmpty(documentList)) {
			...
			return new ResponseEntity(new LamsResponse(HttpStatus.BAD_REQUEST.value(), "Invalid Request, Document id is null or empty"), HttpStatus.OK);
		}
		
		try {
			List<DocumentResponse> documents = documentUploadService.getDocumentList(applicationId, documentList);
			return new ResponseEntity(new LamsResponse(HttpStatus.OK.value(), "Successfully get documentlist",documents), HttpStatus.OK);
		} catch(Exception e) {
			logger.info("Throw Expection while Upload File -------------------->");
			e.printStackTrace();
			return new ResponseEntity(new LamsResponse(HttpStatus.INTERNAL_SERVER_ERROR.value(), "Something went wrong while getting documents"), HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
	...
	public ResponseEntity<LamsResponse> inActiveDocument(@PathVariable("documentMappingId") Long documentMappingId) {
		...
		try {
			...
			if(inactiveDocument) {
				return new ResponseEntity(new LamsResponse(HttpStatus.OK.value(), "Successfully inactive document"), HttpStatus.OK);	
			}
			return new ResponseEntity(new LamsResponse(HttpStatus.BAD_REQUEST.value(), "Invalid Document Mapping Id !!"), HttpStatus.OK);
		} catch(Exception e) {
			logger.info("Throw Expection while inactive document -------------------->");
			e.printStackTrace();
			return new ResponseEntity(new LamsResponse(HttpStatus.INTERNAL_SERVER_ERROR.value(), "Something went wrong while inactive document"), HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}

...
public class ApplicationController {
	...
	public ResponseEntity<LamsResponse> save (@RequestBody ApplicationsBO applicationsBO,HttpServletRequest httpServletRequest){
		...
		if(CommonUtils.isObjectNullOrEmpty(applicationsBO.getLoanTypeId())) {
			return new ResponseEntity<LamsResponse>(new LamsResponse(HttpStatus.BAD_REQUEST.value(), "Loan Type Null Or Empty"),
					HttpStatus.OK);
		}
		applicationsBO.setUserId(userId);
		try {
			applicationsService.save(applicationsBO);
			return new ResponseEntity<LamsResponse>(new LamsResponse(HttpStatus.OK.value(), "Successfully save data", applicationsBO),
					HttpStatus.OK);
		} catch (Exception e) {

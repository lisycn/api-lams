...
import com.lams.api.repository.loan.CarLoanDetailsRepository;
...
@Service
@Transactional
public class CarLoanDetailsServiceImpl implements CarLoanDetailsService{

	public static final Logger logger = Logger.getLogger(CarLoanDetailsServiceImpl.class);
	
	@Autowired
	private CarLoanDetailsRepository repository;
	...
	public Long save(CarLoanDetailsBO requestLoanDetailsBO) {
		...
		if(CommonUtils.isObjectNullOrEmpty(domainObj)) {
			...
			if(!CommonUtils.isObjectNullOrEmpty(requestLoanDetailsBO.getIsFromCP()) && requestLoanDetailsBO.getIsFromCP()) {
				...
				String lastLeadReferenceNo = applicationsRepository.getLastLeadReferenceNoForCP(Long.valueOf(ApplicationType.CAR_LOAN),requestLoanDetailsBO.getLeadReferenceNo());
				domainObj.setLeadReferenceNo(CommonUtils.generateRefNoFromCP(ApplicationTypeCode.CAR_LOAN, lastLeadReferenceNo,requestLoanDetailsBO.getLeadReferenceNo()));
			}else {
				String lastLeadReferenceNo = applicationsRepository.getLastLeadReferenceNo(Long.valueOf(ApplicationType.CAR_LOAN));
				domainObj.setLeadReferenceNo(CommonUtils.generateRefNo(ApplicationTypeCode.CAR_LOAN, lastLeadReferenceNo));				
			}
	...
	public CarLoanDetailsBO get(Long id) {
		CarLoanDetails loanDetails = repository.findByIdAndIsActive(id, true);
		CarLoanDetailsBO response = new CarLoanDetailsBO();

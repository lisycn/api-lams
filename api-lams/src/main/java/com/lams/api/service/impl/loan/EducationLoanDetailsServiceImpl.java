...
@Service
@Transactional
public class EducationLoanDetailsServiceImpl implements EducationLoanDetailsService{

	public static final Logger logger = Logger.getLogger(EducationLoanDetailsServiceImpl.class);
	
	@Autowired
	private EducationLoanDetailsRepository repository;
	...
	public Long save(EducationLoanDetailsBO requestLoanDetailsBO) {
		...
		if(CommonUtils.isObjectNullOrEmpty(domainObj)) {
			domainObj = new EducationLoanDetails();
	...
	public EducationLoanDetailsBO get(Long id) {
		EducationLoanDetails loanDetails = repository.findByIdAndIsActive(id, true);
		EducationLoanDetailsBO response = new EducationLoanDetailsBO();
